...
let createPeerConnection = async (sdpType) => {
    peerConnection = new RTCPeerConnection(servers)//core api
    remoteStream = new MediaStream()
    document.getElementById("user-2").srcObject = remoteStream 

    localStream.getTracks().forEach((track) => {//audio, video tracks
...
let addAnswer = async () => {
   ...
   if(!answer) return 'Retrieve answer from peer first.'
